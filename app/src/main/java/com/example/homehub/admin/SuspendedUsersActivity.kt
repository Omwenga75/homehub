...
import com.example.homehub.databinding.ActivitySuspendedUsersBinding
...
class SuspendedUsersActivity : AppCompatActivity() {

    private lateinit var binding: ActivitySuspendedUsersBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivitySuspendedUsersBinding.inflate(layoutInflater)
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                for (doc in usersSnapshot.documents) {
                    val student = createStudentFromVerification(doc)
                    val displayStatus = student.status.lowercase()
                    if ((displayStatus == "suspended" || displayStatus == "rejected") && isValidStudent(student) && !isDuplicate(student, seenUserIds, seenEmails)) {
    ...
    private fun createStudentFromVerification(document: com.google.firebase.firestore.DocumentSnapshot): Student {
        ...
        val type = (data["userType"] as? String) ?: "Student"
